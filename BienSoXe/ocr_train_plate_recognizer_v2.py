"""Train plate OCR recognizer for Vietnamese car plates (CRNN-CTC).

Windows-compatible version with num_workers=0 and proper collate_fn.
"""
...
class PlateOcrDataset(Dataset):
    def __init__(self, images_dir, labels_dir, img_size=(32, 128)):  # Reduced size
...
# ===== COLLATE FUNCTION (not lambda) =====
def collate_fn_wrapper(batch):
    """Collate batch với padding variable length sequences."""
    images = []
    text_indices = []
    target_lengths = []

    for img, indices in batch:
        images.append(img)  # (1, H, W)
    ...
    # Calculate input_lengths based on output feature map size after CNN
    # After CNN pooling: W' = W / (2 * 2 * 2) for 3 MaxPool2d with stride 2
    # But (2,1) pooling only pools height, not width, so: W' = W / 4
...
def main():
    ...
    # ⚠️ num_workers=0 for Windows compatibility
